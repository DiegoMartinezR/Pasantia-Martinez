...
class pdf_azur extends ModelePDFPropales
{
	...
	function __construct($db)
	{
		...
		if($conf->global->PRODUCT_USE_UNITS)
		{
			$this->posxtva=250;
			$this->posxup=118;   //118
			$this->posxqty=150;		//145
			$this->posxunit=151;	//151
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
					{
						...
						$pdf->MultiCell($this->posxup-$this->posxtva+4, 3, $vat_rate, 0, 'L');
					}
		...
		*/
		...
		if ($deja_regle > 0)
		{
			...
			if ($object->close_code == 'discount_vat')
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, $signo.price($object->total_ttc - $deja_regle, 0, $outputlangs), $useborder, 'R', 1);
